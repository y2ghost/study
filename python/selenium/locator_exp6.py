...
driver.get("https://www.test.com/demo-site/select-dropdown-menu/")
dropdownSelect = Select(
    driver.find_element(By.XPATH, "//div[@class='single_tab_div resp-tab-content resp-tab-content-active']//p//select"))
...
for rowData in range(2, len(numberOfRows) + 1):
    for colData in range(1, len(numberOfCol) + 1):
        rowColData = driver.find_element(By.XPATH, "//table[@id='customers']/tbody/tr[" + str(rowData) + "]/td[" + str(
            colData) + "]")
