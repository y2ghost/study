...
struct scullv_dev *scullv_devices = NULL; /* allocated in scullv_init */
...
{
    struct scullv_dev *dev = NULL; /* device information */
 ...
 */
ssize_t scullv_read (struct file *filp, char __user *buf,
    size_t count, loff_t *f_pos)
{
    struct scullv_dev *dev = filp->private_data; /* the first listitem */
    struct scullv_dev *dptr = NULL;
...
ssize_t scullv_write (struct file *filp, const char __user *buf,
    size_t count, loff_t *f_pos)
{
    ...
    unsigned int cmd, unsigned long arg)
{
    ...
    if (_IOC_TYPE(cmd) != SCULLV_IOC_MAGIC) {
        return -ENOTTY;
    }

    if (_IOC_NR(cmd) > SCULLV_IOC_MAXNR) {
        return -ENOTTY;
    }
...
static ssize_t scullv_aio_read(struct kiocb *iocb, char __user *buf,
...
{
    int err = 0;
    int devno = MKDEV(scullv_major, index);
