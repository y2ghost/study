...
static int skull_find_hw(void) /* returns the # of devices */
{
    ...
    } while (0==skull_port_base && base<SKULL_PORT_CEIL);
...
int skull_init(void)
{
    ...
    unsigned char newval = 0; /* values read from memory   */
    unsigned long flags = 0;          /* used to hold system flags */
    unsigned long add = 0;
    unsigned long i = 0;
    void *base = NULL;
