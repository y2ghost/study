...
struct scullc_dev *scullc_devices = NULL; /* allocated in scullc_init */
    ...
    *start = buf;
    for (i=0; i<scullc_devs; i++) {
        ...
        for (; NULL!=d; d=d->next) { /* scan the list */
            ...
            if (NULL!=d->data && NULL==d->next) { /* dump only the last item - save space */
                for (j=0; j<qset; j++) {
                    if (NULL != d->data[j]) {
...
{
    struct scullc_dev *dev = NULL; /* device information */
 ...
 */
ssize_t scullc_read (struct file *filp, char __user *buf,
    size_t count, loff_t *f_pos)
{
    struct scullc_dev *dev = filp->private_data; /* the first listitem */
    struct scullc_dev *dptr = NULL;
...
ssize_t scullc_write (struct file *filp, const char __user *buf,
    size_t count, loff_t *f_pos)
{
...
static ssize_t scullc_aio_read(struct kiocb *iocb, char __user *buf,
    size_t count, loff_t pos)
{
...
{
    int err = 0;
    int devno = MKDEV(scullc_major, index);
