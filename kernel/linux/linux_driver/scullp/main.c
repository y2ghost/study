...
struct scullp_dev *scullp_devices = NULL; /* allocated in scullp_init */
...
{
    struct scullp_dev *dev = NULL; /* device information */
...
ssize_t scullp_write (struct file *filp, const char __user *buf,
    size_t count, loff_t *f_pos)
{
...
static ssize_t scullp_aio_read(struct kiocb *iocb, char __user *buf,
    size_t count, loff_t pos)
{
...
{
    int err = 0;
    int devno = MKDEV(scullp_major, index);
