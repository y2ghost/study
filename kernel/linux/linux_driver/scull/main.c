...
struct scull_dev *scull_devices = NULL; /* allocated in scull_init_module */
...
int scull_trim(struct scull_dev *dev)
{
    ...
    for (dptr=dev->data; NULL!=dptr; dptr=next) { /* all the list items */
        if (NULL != dptr->data) {
            for (i=0; i<qset; i++) {
                kfree(dptr->data[i]);
            }
...
static int scull_seq_show(struct seq_file *s, void *v)
{
    ...
    for (d=dev->data; NULL!=d; d=d->next) { /* scan the list */
...
{
    struct scull_dev *dev = NULL; /* device information */
