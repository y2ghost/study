        ...
        try {
            client.indices()
                    .create(new CreateIndexRequest(index).mapping(XContentFactory.jsonBuilder().startObject()
                            ...
                            .field("term_vector", "with_positions_offsets").field("store", "true").endObject()
                            .startObject("tag").field("type", "keyword").endObject().endObject().endObject()),
                            RequestOptions.DEFAULT);
        } catch (IOException e) {
            System.out.println("Unable to create mapping");
        }

        client.cluster().health(new ClusterHealthRequest().waitForEvents(Priority.LANGUID).waitForGreenStatus(),
                RequestOptions.DEFAULT);
        BulkRequest bulker = new BulkRequest();

        for (int i = 1; i < 1000; i++) {
            bulker.add(new IndexRequest(index).id(Integer.toString(i)).source("text", Integer.toString(i), "number1",
                    i + 1, "number2", i % 2, "tag", getTag()));
        }
