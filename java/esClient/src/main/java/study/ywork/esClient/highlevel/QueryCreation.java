    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            try {
                client.indices()
                        .create(new CreateIndexRequest(index).mapping(XContentFactory.jsonBuilder().startObject()
                                .startObject("properties").startObject("text").field("type", "integer")
                                .field("store", "true").endObject().startObject("number1").field("type", "integer")
                                .field("store", "true").endObject().startObject("number2").field("type", "integer")
            ...
            for (int i = 1; i < 1000; i++) {
                bulker.add(new IndexRequest(index).id(Integer.toString(i)).source("text", Integer.toString(i),
