...
/* we must add TAILQ_ENTRY field: entries
 * and type name same of structure tag name
 */
struct tailq_entry {
    int value;
...
/* Initialize the tail queue(static). */
TAILQ_HEAD(tailq_head, tailq_entry) my_tailq_head =
    TAILQ_HEAD_INITIALIZER(my_tailq_head);
...
int main(int argc, char **argv)
{
    ...
    /* Initialize the tail queue(dynmic, redundancy for show). */
    ...
    */
    ...
    for (item=TAILQ_FIRST(&my_tailq_head); NULL!=item;
