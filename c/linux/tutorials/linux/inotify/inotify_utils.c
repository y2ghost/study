...
void handle_event(queue_entry_t event)
{
    ...
    event_flags = IN_ALL_EVENTS | IN_UNMOUNT | IN_Q_OVERFLOW | IN_IGNORED;
    flags = event->inot_ev.mask & ~event_flags;
  
    /* Perform event dependent handler routines */
    /* The mask is the magic that tells us what file operation occurred */
    switch (event->inot_ev.mask & event_flags) {
    /* File was accessed */
    case IN_ACCESS:
...
{
    while (1==keep_running && (watched_items_>0)) {
        if (event_check(fd) > 0) {
            int r = read_events(q, fd);
