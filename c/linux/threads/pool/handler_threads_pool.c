...
void add_handler_thread(struct handler_threads_pool* pool)
{
    ...
    struct handler_thread_params* params = NULL;
    ...
    params = (struct handler_thread_params*) malloc(sizeof(*params));
    ...
    pthread_create(&a_thread->thread, NULL, handle_requests_loop, (void*)params);

    /* add the thread's structure to the end of the pool's list. */
    if (0 == pool->num_threads) {
...
static struct handler_thread* remove_first_handler_thread(struct handler_threads_pool* pool)
{
    ...
    /* sanity check */
    assert(pool);
    if (pool->num_threads>0 && NULL!=pool->threads) {
...
{
    struct handler_thread* a_thread = NULL; /* the thread to cancel */
