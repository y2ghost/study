...
static void usageError(char *progName, char *msg, bool shortUsage)
{
    if (msg != NULL) {
        fprintf(stderr, "%s\n", msg);
    }
...
static bool parseEntrySpec(char *entryStr,
    struct AccessControlEntry *ace, bool permsReqd)
{
    ...
    *colon1 = '\0';
    ...
    ace->qual = 0;
    if (ace->tag == ACL_USER) {
        ace->qual = get_uid_byname(colon1 + 1);
        if (ace->qual == -1) {
            fprintf(stderr, "Bad user ID: %s\n", colon1 + 1);
            return false;
        }
    } else if (ace->tag == ACL_GROUP) {
        ace->qual = get_gid_bygname(colon1 + 1);
        if (ace->qual == -1) {
...
static int parseACL(char *aclStr, struct AccessControlEntry aclist[],
        bool permsReqd)
{
    char *comma;
    int n = 0;
...
static acl_entry_t findEntry(acl_t acl, acl_tag_t tag, id_t qaul)
{
    acl_entry_t entry;
    acl_tag_t entryTag;
...
static void setPerms(acl_entry_t entry, int perms)
{
    ...
    if (acl_clear_perms(permset) == -1) {
        err_quit("acl_clear_perms");
    }

    if (perms & ACL_READ) {
        if (acl_add_perm(permset, ACL_READ) == -1) {
            err_quit("acl_add_perm");
        }
    }
    ...
    if (acl_set_permset(entry, permset) == -1) {
        err_quit("acl_set_permset");
    }
}

static void updateACL(char *file, bool useDefaultACL,
...
int main(int argc, char *argv[])
{
    ...
    if (operation == checkValidACL) {
        if (parseACL(aclSpec, aclist, true) == -1) {
            err_quit("Bad ACL entry specification");
        } else {
            printf("ACL is valid\n");
            exit(EXIT_SUCCESS);
        }
    ...
    for (int j = optind; j < argc; j++) {
        if (operation == removeDefaultACL) {
            if (acl_delete_def_file(argv[j]) == -1) {
                err_quit("acl_delete_def_file: %s", argv[j]);
            }
        } else if (operation == modifyEntries || operation == removeEntries) {
            updateACL(argv[j], useDefaultACL, numEntries, aclist, operation, recalcMask);
        } else {
            err_quit("Bad logic!");
        }
    }
