...
void print_timing(char *name, AVFormatContext *avf, AVCodecContext *avc, AVStream *avs)
{
  ...
  if (avf != NULL) {
    logging("\t\tstart_time=%d duration=%d bit_rate=%d start_time_realtime=%d",
      avf->start_time, avf->duration, avf->bit_rate, avf->start_time_realtime);
  } else {
    logging("\t\t->NULL");
  }
  ...
  if (avc != NULL) {
    logging("\t\tbit_rate=%d width=%d height=%d gop_size=%d ",
      avc->bit_rate, avc->width, avc->height, avc->gop_size);
    logging("keyint_min=%d sample_rate=%d profile=%d level=%d ",
      avc->keyint_min, avc->sample_rate, avc->profile, avc->level);
    ...
    logging("\t\tavc->pkt_timebase=num/den %d/%d",
      avc->pkt_timebase.num, avc->pkt_timebase.den);
  } else {
    logging("\t\t->NULL");
  }
